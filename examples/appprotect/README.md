...
4. Create an Ingress resource:
    Get the previously deployed syslog IP:
    ```
    $ kubectl get ep syslog-svc
    ```
    Update the `appprotect.f5.com/app-protect-security-log-destination` annotation from `cafe-ingress.yaml` with the IP. For example, if the IP is `172.17.0.11`:
    ```yaml
    . . .
    appprotect.f5.com/app-protect-security-log-destination: "syslog:server=172.17.0.11:514"
    ```
